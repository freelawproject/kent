...
class _NoopHandle:
    """HTTP holds no per-worker resource (httpx pools internally)."""
...
class _HttpArchiveStream:
    ...
    def __init__(self, cm: Any, streaming: _AsyncStreamingResponse) -> None:
        ...
        self.status_code: int = streaming.status_code
        self.headers: dict[str, str] = streaming.headers
        self.url: str = streaming.url
...
class HttpxTransport:
    ...
    def _make_client(self, verify: bool | str) -> httpx.AsyncClient:
        ...
        return httpx.AsyncClient(
            verify=verify, timeout=self._timeout, proxy=self._proxy
        )
    ...
    async def _stream_request(
        ...
        try:
            async with client.stream(
                ...
                yield _AsyncStreamingResponse(http_response, http_params.url)
                logger.info(
