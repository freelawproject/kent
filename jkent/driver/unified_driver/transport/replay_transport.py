...
def _decode(content: bytes, headers: dict[str, str]) -> str:
    """Decode body to text exactly as the live HttpxTransport would.

    The live path sets ``Response.text = httpx.Response.text``, so reconstructing
    an :class:`httpx.Response` from the stored bytes + headers and reading its
    ``.text`` reuses httpx's full charset resolution (declared content-type
    charset, then httpx's default, with BOM handling). Replayed ``Response.text``
    is therefore identical by construction to what the original run produced —
    no hand-rolled charset parsing to drift from httpx's behavior.
    """
    return httpx.Response(200, headers=headers, content=content).text


class ReplayTransport(Transport[NoopHandle]):
    ...
    def _lookup_entry(self, request: Request) -> IndexEntry | None:
        """Two-tier lookup: own dedup key, then the URL+body fallback key.

        The single matching contract shared by :meth:`resolve` and
        :meth:`resolve_archive` so both paths match the same set of stored rows.
        """
    ...
    def _note_retry_eligible(self, entry: IndexEntry, request_id: int) -> None:
        """Record a served retry-eligible parent for ReplayWorker force-miss.

        Called on every serving path (response *and* archive) so a retry-eligible
        archive parent's children get force-missed just like a response parent's.
        """
    ...
    async def resolve(
        ...
        self._note_retry_eligible(entry, queued.request_id)
        fetched = await asyncio.to_thread(
            self._require_index().fetch_response, entry
        )
        return Response(
    ...
    async def resolve_archive(
        ...
        entry = self._lookup_entry(request)
        fetched = (
            await asyncio.to_thread(self._require_index().fetch_archive, entry)
        ...
        # Mirror resolve(): archive rows can be retry-eligible too, so record the
        # parent now that we're committed to serving it.
        self._note_retry_eligible(entry, queued.request_id)
    ...
    # finish_archiving: inherited no-op from Transport — the stored archive
    # file belongs to the source DB, so there is nothing to release.
