...
divergence is expected by design (disabled controls, extra fields,
non-first submit buttons) the differential tests target it deliberately.
"""
...
# browser never submits a disabled control. ``find_form`` currently does not
# check ``disabled``, so these isolate that divergence.
...
class Control:
    ...
    def render(self) -> str:
        ...
        if self.kind in (SELECT, MULTISELECT):
            ...
            return (
                f'<select id="{i}" name="{n}"{multiple}>{opts}</select>'
            )
    ...
    def native_fill(self) -> tuple[str, str] | None:
        """A ``(name, value)`` the oracle types natively, or ``None``."""
        if self.kind in (TEXT, TEXTAREA) and self.fill:
            return (self.name, self.value)
        return None
...
class FormCase:
    ...
    def overrides(self) -> dict[str, str | list[str]]:
    ...
    def native_fills(self) -> list[tuple[str, str]]:
        """The fields the oracle browser types into before submitting."""
